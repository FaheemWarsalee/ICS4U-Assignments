...
void draw() {
  ...
  if (playingGame) {
    ...
    {//Shows things necessary for playing the game
      inMenu = false;
      menuTheme.stop();
...
void keyPressed() {
  if (playingGame) {
    ...
    if (inMenu) {
      ...
      } else if (keyCode == ENTER && beatGame) {
...
void playGame() {
  ...
  stats = new Stats(player.atk, player.hp, statsImg);
...
/*checks to see if the player is on the bosster and uses it
 pre: none
 post: none */
...
/*sorts the scoreboard from lowest to highest
 pre: none
 post: none */
...
/*sets boosters all around the map
 pre: none
 post: none */
void setBoosters() {
  int rngGridLoc;
  for (int i = 0; i < boosters.length; i ++) {
    ...
    while (rngGridLoc ==6);
    if (boostType == 1) {
      boosters[i] = new Booster((int)random(1, 18) * 50, (int)random(1, 18) * 50, 50, 50, rngGridLoc, boosterImg, boostType); // Creates boosters at random locations throughout the map, but only in increments of 50
    } else {
      boosters[i] = new Booster((int)random(1, 18) * 50, (int)random(1, 18) * 50, 50, 50, rngGridLoc, healthBoosterImg, boostType); // Creates boosters at random locations throughout the map, but only in increments of 50
    }
